...
object Impute3 {
  ...
  val lines = spark.sparkContext.textFile("file:///D:/task/VehicleNoNotAvailable.csv")
    ...
    val filteredSourceVals = sourceVals.filter(x => (x.matches(pattern)))
      .filter(x => (x.length() > 10 && x.length() < 40)).map(x=>(x,1))
     // .collect.toSet
    ...
    val txnVals = txnLines.map(x => x.split(",")).filter(x => x.length >= 2)
      .map(x =>
        if (x.length == 2)
          ...
          (x(0), (x(1), x(2))))
           .partitionBy(new HashPartitioner(210))
/*
     ...
     */
          ...
          println(txnVals.partitions.length)
          println(filteredSourceVals.partitions.length)
     val joined=   txnVals.join( filteredSourceVals).mapValues(x=>x._1) 
     
   //   .persist()
      
      ///
       println(joined.partitions.length)
...
/*
    ...
    val properFormatRDD = joined.mapValues(x => (x._1)).filter(x => (x._2.matches(format))).distinct().persist(StorageLevel.MEMORY_AND_DISK)

    val multiVehicleGroup = properFormatRDD.groupByKey().filter(x => x._2.size > 1).mapValues(x => (x.toList)).persist(StorageLevel.MEMORY_AND_DISK)
  ...
  def writeToCSV(df: DataFrame, file: String): Unit = {
    val folder = "file:///D:/task/Impute/findoutVehicle/"
