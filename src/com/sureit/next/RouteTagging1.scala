...
object RouteTagging1 {
    ...
    val pairsArray=
      for(
        ...
        for (i<-0 to tag._2.length-3) yield {
          (tag._2(i).plaza,tag._2(i).exit,tag._2(i+1).plaza,tag._2(i+1).entry,Duration.between(tag._2(i+1).time, tag._2(i).time).toHours())
        }
  ...
  //  println("----------"+pairs.filter(x=>(x._3!==x._4)).count)
   val pairswithSet=pairs .map(x=>((x._1,x._2,x._3),(1.toLong,x._5,Set(x._4))))
    ...
    .map(x=>(x._1._1,x._1._2,x._2._1,x._2._2,x._2._3,(x._2._2*100/x._2._3),x._2._4/x._2._4,x._2._5))
  ...
  spark.sparkContext.textFile("file:///D:/task/data/TagPlazaCodeEntryExitLaneTime-sample.csv")
   ...
   .map(_.split(","))
   .map(x=>(x(0),x(1),x(2),x(3),LocalDateTime.parse(x(4),DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSS"))))
   .map(x => (x._1,  TreeSet(txn(x._2, x._3, x._4,x._5))))
    
  }
  
  def writeToCSV(df: DataFrame, file: String): Unit = {
    val folder = "file:///D:/task/RouteTagging/2/"
