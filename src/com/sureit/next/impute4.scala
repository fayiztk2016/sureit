...
object Impute4 {
  ...
  def main(args: Array[String]) {
    ...
    val improperRegNo = spark.sparkContext.textFile("file:///D:/task/data/imroperMapRegisterNumber.txt")
    val improperRegNoRDD =
      improperRegNo.map(x => x.split(",")).filter(x => x.length > 0).map(x => (x(0), x(1)))

    val format = "^[a-zA-Z]{2}[0-9]+[A-Z0-9]+$"
    val stateCode = spark.sparkContext.textFile("file:///D:/task/data/StateCodeName.txt").map(x => x.split(",")).map(x => (x(0))) 
     .collect.to
    val validRegNo = improperRegNoRDD.filter(x => (x._2.matches(format))).filter(x=>stateCode.contains(x._2.substring(0, 1)))
        println(validRegNo.count)
    /*
